...
- (void)applyFileFilter:(NSString *)searchString {
    ...
    NSIndexSet *norm = [searchStringArray indexesOfObjectsPassingTest:^(id obj, NSUInteger index, BOOL *stop) {
        ...
        while(index != NSNotFound) { 
            dates[index] = [self dateFromString:[[searchStringArray objectAtIndex:index] substringFromIndex:2]];
            index = [ge indexGreaterThanIndex:index]; 
        }
        index = [greater firstIndex]; 
        while(index != NSNotFound) { 
            dates[index] = [[self dateFromString:[[searchStringArray objectAtIndex:index] substringFromIndex:1]] endDay];
            index = [greater indexGreaterThanIndex:index]; 
        }
        index = [le firstIndex];     // map to <
        while(index != NSNotFound) { 
            dates[index] = [[self dateFromString:[[searchStringArray objectAtIndex:index] substringFromIndex:2]] nextDay];
            index = [le indexGreaterThanIndex:index]; 
        }
        index = [less firstIndex]; 
        while(index != NSNotFound) { 
            //            times[index] = [self getDate:[[searchStringArray objectAtIndex:index] substringFromIndex:1]];
            dates[index] = [self dateFromString:[[searchStringArray objectAtIndex:index] substringFromIndex:1]];
